package com.gerantech.towercraft.controls.screens
{
	...
	public class BattleScreen extends BaseCustomScreen
	{
		...
		private function endBattle(data:SFSObject):void
		{
			...
			{
				if( appModel.game.player.inFriendlyBattle )
					GameAnalytics.addProgressionEvent((endOverlay.score>0)?GAProgressionStatus.COMPLETE:GAProgressionStatus.FAIL, field.isQuest?"Quests":"Battles", "FriendlyBattle", field.index.toString());
				else
					GameAnalytics.addProgressionEvent((endOverlay.score>0)?GAProgressionStatus.COMPLETE:GAProgressionStatus.FAIL, field.isQuest?"Quests":"Battles", field.isQuest?"Quests":"Battles", field.index.toString(), endOverlay.score);
				var keys:Vector.<int> = outcomes.keys();
				for each (var k:int in keys)
				GameAnalytics.addResourceEvent(outcomes.get(k)>0?GAResourceFlowType.SOURCE:GAResourceFlowType.SINK, k.toString(), outcomes.get(k), "outcome", field.isQuest?"quests":"battles");
			}
		}
