...
protected function sfsConnection_extensionResponseHandler(event:SFSEvent):void
{
	...
	if( item.isBook() || item.containBook() > -1 )
	{
		...
		for( var i:int=0; i<data.getSFSArray("rewards").size(); i++ )
		{
			...
			if( ResourceType.isBuilding(reward.getInt("t")) || ResourceType.isBook(reward.getInt("t")) || reward.getInt("t") == ResourceType.CURRENCY_SOFT || reward.getInt("t") == ResourceType.CURRENCY_HARD || reward.getInt("t") == ResourceType.TICKET )
