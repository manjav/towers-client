...
private function showMidSwipesTutorial(troopType : int) : void
{
	...
	if( place.building.troopType == player.troopType || troopType != player.troopType )
		return;
	if( appModel.battleFieldView.battleData.battleField.field.isOperation && appModel.battleFieldView.battleData.battleField.field.index == 2 && player.emptyDeck() )
		return;
	tutorials.removeAll();
	if( place.index > appModel.battleFieldView.battleData.battleField.field.places.size() - 2 )
		return;
...
{
	var p:PlaceData = appModel.battleFieldView.battleData.battleField.field.places.get(index);
