...
override protected function addedToStageHandler(event:Event):void
{
	...
	titleDisplay = new ShadowLabel("", 1, 0, "left", null, false, null, 1.3);
...
private function pullCard() : void
{
	...
	// play SFXs
	if( ResourceType.isBuilding(rewardType) )
...
private function showDetails() : void 
{
	...
	titleDisplay.text = loc(( ResourceType.isBuilding(rewardType) ? "building_title_" : "resource_title_" ) + rewardType);
	titleDisplay.x = 480;
	Starling.juggler.tween(titleDisplay, 0.3, {x:540, transition:Transitions.EASE_OUT_BACK});
	addChildAt(titleDisplay, 1);
	
	if( ResourceType.isBuilding(rewardType) && rewardRarity > 0 )
	{
		descriptionDisplay.elementFormat = new ElementFormat(descriptionDisplay.fontDescription, descriptionDisplay.fontSize, 1);
